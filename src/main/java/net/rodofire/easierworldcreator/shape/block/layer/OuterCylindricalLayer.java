...
public class OuterCylindricalLayer extends AbstractOuterLayer {
    OuterCylindricalLayer(BlockLayerManager blockLayer, Vec3d center, Vec3i direction) {
    ...
    protected float getDistance(int[] pos) {
        return maxDistance - WorldGenUtil.getDistanceToAxis(this.centerPos, this.directionVector, new Vec3d(pos[0], pos[1], pos[2]));
    }

    @Override
    protected float getDistance(int posX, int posY, int posZ) {
        return maxDistance - WorldGenUtil.getDistanceToAxis(this.centerPos, this.directionVector, new Vec3d(posX, posY, posZ));
    }
