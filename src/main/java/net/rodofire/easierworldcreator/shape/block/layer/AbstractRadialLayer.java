...
public abstract class AbstractRadialLayer extends AbstractLayer {
    ...
    public BlockListManager get(Map<ChunkPos, LongOpenHashSet> posMap) {
        ...
        int[] center = LongPosHelper.decodeBlockPos2Array(this.centerPos);
        for (LongOpenHashSet set : posMap.values()) {
            futures.add(CompletableFuture.runAsync(() -> {
                BlockListManager manager1 = new BlockListManager();
                for (long pos : set) {
                    float distance = WorldGenUtil.getDistance(center[0], center[1], center[2], LongPosHelper.decodeBlockPos2Array(pos));
    ...
    public <T extends Collection<BlockPos>> BlockListManager get(T posList) {
        ...
        for (BlockPos pos : posList) {
            futures.add(CompletableFuture.runAsync(() -> {
                float distance = WorldGenUtil.getDistance(center[0], center[1], center[2], pos.getX(), pos.getY(), pos.getZ());
                int index = findLayerIndex(layerDistance, distance);
                BlockLayer layer = (index >= 0) ? blockLayer.get(index) : blockLayer.get(0);
