...
public class InnerRadialLayer extends AbstractRadialLikeLayer {
    InnerRadialLayer(BlockLayerManager blockLayer, Vec3 center) {
    ...
    protected float getDistance(int[] pos) {
        return WorldGenUtil.getDistance(this.centerPos, new Vec3(pos[0], pos[1], pos[2]));
    }

    @Override
    protected float getDistance(int posX, int posY, int posZ) {
        return WorldGenUtil.getDistance(this.centerPos, new Vec3(posX, posY, posZ));
    }
}
