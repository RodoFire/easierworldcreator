...
public class InnerCylindricalLayer extends AbstractRadialLikeLayer {
    InnerCylindricalLayer(BlockLayerManager blockLayer, Vec3 center, Vec3i direction) {
    ...
    protected float getDistance(int[] pos) {
        return  WorldGenUtil.getDistanceToAxis(this.centerPos, this.directionVector, new Vec3(pos[0], pos[1], pos[2]));
    }

    @Override
    protected float getDistance(int posX, int posY, int posZ) {
        return  WorldGenUtil.getDistanceToAxis(this.centerPos, this.directionVector, new Vec3(posX, posY, posZ));
    }
